...
def discrete_adaptive_control(x0, x0_dot, xm0, xm0_dot, ones, t, discrete_control_rate, Improvement=False):
    ...
    for index, control_rate in enumerate(discrete_control_rate):
        ...
        path = f'images/Q5/results_for_{control_rate}hz'
        if path:
            ...
            plt.plot(t, np.array(xm_lst), label=r"$x_r(t)$", linewidth=2)
            plt.plot(t, np.array(x_lst), label=r"$x(t)$", linewidth=2)
            ...
            plt.legend(prop={'size': 17}, framealpha=1)
            plt.grid()
            plt.savefig(f'{path}/Step Error.png')
            plt.close()

            plt.figure(figsize=(14, 6))
            plt.xlabel("t [sec]", fontsize=20)
            plt.ylabel("Input", fontsize=20)
            plt.plot(t, input_lst, label=r"$u(t)$", linewidth=2)
            plt.legend(prop={'size': 17}, framealpha=1)
            plt.grid()
            plt.savefig(f'{path}/Step Input.png')
            plt.close()
    ...
    plt.plot(t, np.array(xm_lst), label=r"$x_r(t)$", linewidth=2)
    for index, control_rate in enumerate(discrete_control_rate):
        plt.plot(t, np.array(x_rate[:, index]), label=f'x(t) {control_rate}hz', linewidth=2)
    plt.legend(prop={'size': 17}, framealpha=1)
    plt.title("All Rates Tracking performance", fontsize=24)
    plt.grid()
    plt.savefig(f'images/Q5/All Rates Reference Tracking performance.png')
    plt.close()
    ...
    plt.ylabel("e(t)", fontsize=20)
    for index, control_rate in enumerate(discrete_control_rate):
        plt.plot(t, np.array(err_lst[:, index]), label=f'Error {control_rate}hz', linewidth=2)
    plt.legend(prop={'size': 17}, framealpha=1)
    plt.grid()
    plt.savefig(f'images/Q5/All Rates Error.png')
    plt.close()
...
discrete_adaptive_control(x0, x0_dot, xm0, xm0_dot, ones, t, discrete_control_rate)
